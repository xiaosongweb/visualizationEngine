...
核心编辑区域，采用 **网格自动排序布局 (Grid & Auto-layout)**，类似于 Grafana 的仪表盘布局体验。

- **布局与交互 (Layout & Interaction)**:
  - **Grid System**: 采用响应式网格系统，组件在网格中自动排列、紧凑堆叠。
  - **自动排序 (Auto-sorting)**:
    - **Push & Compact**: 拖拽组件时，其他组件自动避让（挤开）并重新排列，确保画布无空洞。
    - **Resize**: 调整大小时，周围组件自动适配位置。
  - **磁吸 (Snap)**: 组件自动吸附到网格单元，对齐更轻松。
- **组件操作 (Component Actions)**:
  - **Hover Toolbar**: 鼠标悬停在组件上时，显示悬浮工具栏。
    - **编辑 (Edit)**: 选中组件，右侧属性面板显示对应配置。
    - **删除 (Delete)**: 从画布中移除该组件，触发自动重新布局。
    - **拖拽句柄 (Drag Handle)**: 顶部/角落提供专门的拖拽区域。
- **缩放与漫游 (Zoom & Pan)**:
  - **缩放范围**: `10%` (Min) - `400%` (Max)。
  - **交互**:
